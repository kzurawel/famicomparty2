...
class Emulator {
	...
	bindKeys() {
		...
		document.querySelector('canvas').addEventListener('keydown', this.boundKeyDown);
		document.querySelector('canvas').addEventListener('keyup', this.boundKeyUp);
	}

	unbindKeys() {
		document.querySelector('canvas').removeEventListener('keydown', this.boundKeyDown);
		document.querySelector('canvas').removeEventListener('keyup', this.boundKeyUp);
	}
...
class Video {
	constructor(module, e, el) {
		...
		this.rgbaBuffer = new Uint32Array(
...
class WebGLRenderer {
	constructor(el, palette) {
		...
		gl.bufferData(
			gl.ARRAY_BUFFER,
			new Float32Array([-1, -1, 0, h, +1, -1, w, h, -1, +1, 0, 0, +1, +1, w, 0]),
			gl.STATIC_DRAW
		);
